...
/*!************************************************!*\
  !*** ./resources/js/app/newbooktravelEvent.js ***!
  \************************************************/
$(function () {
  operateFormatter = function operateFormatter(value, row, index) {
    return ['<button type="button"  id="btndp_' + index + '" class="btn btn-success btn-sm btnpaytravel" ><i class="fas fa-cash-register"></i> Bayar</button>', '&nbsp', '<a href="/detailbooktravel/' + row.id + '"  id="btnres_' + index + '" class="btn btn-info btn-sm" ><i class="fas fa-info"></i> Detail</a>'].join('');
  };
});
window.operateEvents = {
  'click .btnpaytravel': function clickBtnpaytravel(e, value, data, index) {
    var _this = this;
